...
ANONYMOUS_ID = int(os.getenv("ANONYMOUS_ID")) # Register a dummy account that users can use for commenting anonymously with registering themselves
SUPER_ID = int(os.getenv("SUPER_ID")) # The super user's ID that can edit other admin's content and delete every comment
...
def home():
    ...
    result = db.session.execute(db.select(Post).order_by(Post.create_date.desc()).where(Post.deleted == False).limit(posts_per_page).offset(offset)).scalars().all()
...
def show_post(post_title):
    ...
    result = db.session.execute(db.select(Comment).where(Comment.post_id == post.id, Comment.deleted == False)).scalars().all()
    ...
    if comment_form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            return redirect(url_for("login"))
    return render_template("post.html", post=post, comments=comments, form=comment_form, anonymous_gravatar=random_gravatar_url, calculate_reading_time=calculate_reading_time,
